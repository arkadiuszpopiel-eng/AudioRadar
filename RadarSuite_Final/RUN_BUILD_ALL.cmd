...
pip install scipy >> "%LOG_FILE%" 2>&1

echo    - Installing sounddevice...
echo [%date% %time%] Installing sounddevice... >> "%LOG_FILE%"
pip install sounddevice >> "%LOG_FILE%" 2>&1

echo    - Installing soundcard...
echo [%date% %time%] Installing soundcard... >> "%LOG_FILE%"
pip install soundcard >> "%LOG_FILE%" 2>&1

echo    - Installing psutil...
echo [%date% %time%] Installing psutil... >> "%LOG_FILE%"
...
pyinstaller --clean --noconfirm build_tools\radarsuite.spec >> "%LOG_FILE%" 2>&1
...
echo    - EXE location: %APP_DIR%\RadarSuite_Final.exe
echo [%date% %time%] EXE created: %APP_DIR%\RadarSuite_Final.exe >> "%LOG_FILE%"
...
echo   - EXE File: RadarSuite_Final.exe
echo   - EXE Size: !EXE_SIZE! bytes
if exist "dist\%ZIP_NAME%" (

...
* **Logging** to a versioned `log_<version>.txt` file (e.g. `log_v9.txt`) so you can troubleshoot audio problems or
...
3. Uruchom `RUN_BUILD_ALL.cmd`. Skrypt ten zainstaluje brakujące
   ...
   numer wersji, np. `log_v9.txt`. W pliku tym zapisana jest
...
* Po uruchomieniu program wypisuje w konsoli ścieżkę, do której zapisywany jest plik logu (np. `log_v9.txt` w folderze `audio_radar`). Jeśli szukasz logu w niewłaściwym miejscu, może wydawać się, że się nie tworzy. Sprawdź lokalizację podaną w komunikacie `[AudioRadar v9] Logging to …`.
